...
bool isCenterDetected()
{
  ...
  return (centerVal > THRESHOLD);
}
bool isRightDetected()
{
  rightVal = analogRead(RIGHT_SENSOR);

  return (rightVal > THRESHOLD);
}
bool isLeftDetected()
{
  leftVal = analogRead(LEFT_SENSOR);

  return (leftVal > THRESHOLD);
}
...
bool isThreeOn()
{
  ...
  centerVal = analogRead(CENTER_SENSOR);
  rightVal = analogRead(RIGHT_SENSOR);
  return leftVal > THRESHOLD && rightVal > THRESHOLD && centerVal > THRESHOLD;
}
...
bool isOnlyCenter()
{
  ...
  return centerVal > THRESHOLD && leftVal < THRESHOLD && rightVal < THRESHOLD;
}